...
class HistoryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final summaryAsync = ref.watch(healthSummaryProvider(period.name));
    ...
    return SafeArea(
      child: RefreshIndicator(
        ...
        onRefresh: () => ref.read(syncManagerProvider(userId)).drain(),
        child: ListView(
          padding: const EdgeInsets.all(20),
          children: [
            Text('History', style: NocturneType.h4),
            const SizedBox(height: 14),
            NocturneSegmentedControl<HistoryPeriod>(
              value: period,
              options: const [
                NocturneSegmentOption(label: 'Daily', value: HistoryPeriod.daily),
                NocturneSegmentOption(label: 'Weekly', value: HistoryPeriod.weekly),
              ],
              onChanged: (p) => ref.read(selectedHistoryPeriodProvider.notifier).select(p),
            ),
